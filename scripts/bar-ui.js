(function(window) {
  ...
  var Player,
      ...
      playerOptions,
      utils,
	  soundhref; // HACK: Logical flag for Play/Pause
  ...
  Player = function(playerNode) {
    ...
    function setTitle(item) {
      ...
      if (links.length) {
        item = links[1]; // HACK: Is [1] because there are two tags <a> in each <li>
      }
    ...
    function playLink(link) {
      ...
      if (soundManager.canPlayURL(link.href)) {
        ...
        //stopOtherSounds(); 
		soundManager.stopAll(); // Stop cualquier evento soundManager
		...
		soundhref = link.href; // HACK: Logical flag for Play/Pause
			soundObject.play({
			url: link.href,
			position: 0
        });
    ...
    function PlaylistController() {
      ...
      function getNext() {
        ...
        if (data.playlist.length > 0) {// HACK: There are not initial <li> therefore it is not 1 is 0
      ...
      function getURL() {
        ...
        if (item) {
          url = item.getElementsByTagName('a')[1].href; // // HACK: Is [1] because there are two tags <a>
        }
    ...
    function handleMouseDown(e) {
      ...
      if (target.nodeName.toLowerCase() !== 'a') {
        ...
        if (links && links.length) {
          target = target.getElementsByTagName('a')[1]; // HACK: Is [1] because there are two tags <a>
        }
        ...
        if (targetNodeName === 'a') {
          ...
          if (soundManager.canPlayURL(href)) {
            ...
            if (!utils.css.has(target, playerOptions.excludeClass)) {
				...
				// HACK: Paused if the same song is already playing
				if ( href == soundhref ) {
    ...
    actions = {
      ...
      play: function(offsetOrEvent) {
        ...
        if (!href || href.indexOf('#') !== -1) {
          href = dom.playlist.getElementsByTagName('a')[1].href; // HACK: Is [1] because there are two tags <a>
        }

        if (!soundObject) {
          soundObject = makeSound(href);
		  soundhref = href; // HACK: Logical flag for Play/Pause
        }
      ...
      next: function(/* e */) {
        ...
        if (item && playlistController.data.selectedIndex !== lastIndex) {
          playLink(item.getElementsByTagName('a')[1]); // HACK: Is [1] because there are two tags <a> in each <li>
        }
      },
